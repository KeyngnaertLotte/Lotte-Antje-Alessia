<template>
  <div class="block fixed z-1 left-0 top-0 w-screen h-screen bg-black bg-opacity-50 flex justify-center items-center ">
      <div class="relative bg-white w-9/10 h-3/4 rounded-lg flex flex-col  items-center p-6">
          ...
          <div class="flex flex-col justify-center items-center lg:flex-row h-auto lg:h-full">
          <img :src=currentArtist.imgLink alt="aaaaaaaaaaaaaaa" class="my-6 lg:mx-10 lg:w-1/3">
          <div>
          <p class="font-pop text-lg text-justify">{{ $t(`detail.info.${currentArtist.artistName.replace(/\s+/g, '.').toLowerCase()}`)}}</p>
          <br>
          <p class="font-pop text-lg text-justify"> {{ $t(`detail.top.${currentArtist.artistName.replace(/\s+/g, '.').toLowerCase()}`)}} </p>
        </div>
        </div>
      </div>
  </div>
